...
export const BabylonProvider = (props: {
  children: ReactNode;
}) => {
  ...
  const showSceneCapture = async () => {
    await delay(500);
    currentScene.current = APP_SCENE.CAPTURE;
    stopCameras();
  };
  const showSceneCombat = async () => {
    await delay(500);
    currentScene.current = APP_SCENE.COMBAT;
    stopCameras();
  };
  ...
  useEffect(() => {
    ...
    sceneDetails.metadata = {
      name: 'SceneDetails'
    };
    sceneCapture.metadata = {
      name: 'SceneCapture'
    };
    sceneCombat.metadata = {
      name: 'SceneCombat'
    };
    ...
    new Babylon.HemisphericLight('light', new Babylon.Vector3(0, 1, 0), sceneCapture);
    new Babylon.HemisphericLight('light', new Babylon.Vector3(0, 1, 0), sceneCombat);
  ...
  useEngineRenderLoop(engine, () => {
    DEBUG_RENDER_LOOP && console.log('DEB: currentScene', currentScene.current);
    switch (currentScene.current) {
      case APP_SCENE.MAP:
  ...
  return <BabylonContext.Provider value={{
    engine,
    sceneMap,
    sceneDetails,
    sceneCapture,
