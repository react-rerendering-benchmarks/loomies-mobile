...
import { getLoomieTeamService, getLoomiesRequest } from '@src/services/loomies.services';
...
export const UserLoomies = ({
  ...
  const fetchLoomies = async () => {
    ...
    if (!err) {
      ...
      const loomiesWithTeamProperty = loomies.map(loomie => ({
        ...loomie,
        is_in_team: team.current.includes(loomie._id)
      }));
  ...
  const fetchLoomieTeam = async () => {
    ...
    team.current = team.map(loomie => loomie._id);
  };
  ...
  const goToLoomieTeam = useCallback(() => {
    navigation.navigate('UpdateLoomieTeam');
  }, []);
  const goToDetails = useCallback((loomieId: string) => {
    const loomie = loomies.find(loomie => loomie._id === loomieId);
    if (!loomie) return;
    navigation.navigate('LoomieDetails', {
      loomie
    });
  }, [loomie, loomies]);
  ...
  const renderLoomies = memo(() => {
    if (!loading && loomies.length === 0) return <EmptyMessage text="You don't have any Loomies yet..." showButton={true} buttonText={'Catch Loomies'} buttonCallback={goToMap} />;
    return <LoomiesGrid loomies={loomies} markBusyLoomies={false} markSelectedLoomies={false} elementsCallback={goToDetails} />;
  });
  return <View style={{
    position: 'relative',
    flex: 1
      ...
      {!loading && loomies.length > 0 && <FloatingRedIcon onPress={goToLoomieTeam} collection='MaterialCommunityIcons' name='sword' bottom={16} right={16} />}
    </View>;
};