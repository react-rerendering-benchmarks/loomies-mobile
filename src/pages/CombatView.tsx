...
import { iCombatMessage, iPayload_START, iPayload_UPDATE_USER_LOOMIE_HP, iLoomie, TYPE, iPayload_UPDATE_PLAYER_LOOMIE, iPayload_GYM_LOOMIE_WEAKENED, iPayload_USER_LOOMIE_WEAKENED, iPayload_USER_ITEM_USED, iPayload_ERROR_USING_ITEM, iPayload_LOOMIES_TEAM } from '@src/types/combatInterfaces';
...
export const CombatView = ({
  ...
  const modalLooseToggle = (open?: boolean) => {
    if (open != undefined) setModalLooseVisible(open);else setModalLooseVisible(value => !value);
  };
  const modalWinToggle = (open?: boolean) => {
    if (open != undefined) setModalWinVisible(open);else setModalWinVisible(value => !value);
  };
  ...
  useEffect(() => {
    ...
    switch (TYPE[messageType]) {
      ...
      case TYPE.UPDATE_USER_LOOMIE_HP:
        {
          ...
          setLoomiePlayer(loomie => {
            if (loomie) {
              ...
              if (payload.damage > 0) queueMessage(payload.was_critical ? `-${payload.damage} Effective attack!` : `-${payload.damage}`, false);
      ...
      case TYPE.UPDATE_GYM_LOOMIE_HP:
        {
          ...
          setLoomieGym(loomie => {
            if (loomie) {
              ...
              if (payload.damage > 0) queueMessage(payload.was_critical ? `-${payload.damage} Effective attack!` : `-${payload.damage}`, true);
      ...
      case TYPE.UPDATE_USER_LOOMIE:
        {
          if ((data.payload as iPayload_UPDATE_PLAYER_LOOMIE) === undefined) return;
      ...
      case TYPE.GYM_LOOMIE_WEAKENED:
        {
          if ((data.payload as iPayload_GYM_LOOMIE_WEAKENED) === undefined) return;
      ...
      case TYPE.USER_LOOMIE_WEAKENED:
        {
          if ((data.payload as iPayload_USER_LOOMIE_WEAKENED) === undefined) return;
  ...
  const userAttack = useCallback(() => {
    const message = JSON.stringify({
      type: (TYPE[TYPE.USER_ATTACK] as string)
    });
    sendMessage(message, false);
  }, [message]);
  const userDodge = useCallback((_direction: boolean) => {
    const message = JSON.stringify({
      type: (TYPE[TYPE.USER_DODGE] as string)
    });
    sendMessage(message, false);
  }, [message]);
  const userEscape = useCallback(() => {
    const message = JSON.stringify({
  ...
  const userLoomiesTeam = useCallback(() => {
    const message = JSON.stringify({
      type: (TYPE[TYPE.USER_GET_LOOMIE_TEAM] as string)
    });
    sendMessage(message, false);
  }, [message]);
  const changeLoomie = useCallback((newLoomie: iLoomie) => {
  ...
  const queueMessage = useCallback((message: string, direction: boolean) => {
    let currentId = 0;
  ...
  }, [message, direction, setQueueUpdated]);
  const getMessageQueue = useCallback((): iDisplayMessage[] => displayMessageQueue.current, []);
  const removeMessageFromQueue = useCallback((deletedIds: number[]) => {
    // filter deleted ids

    displayMessageQueue.current = displayMessageQueue.current.filter(msg => {
      return undefined == deletedIds.find(id => {
        return id == msg.id;
      });
    });
  }, []);
  ...
  const applyItem = useCallback((itemid: string) => {
    const message = JSON.stringify({
  ...
  return <>
      ...
      backgroundColor: 'black'
        ...
        flex: 1
          ...
          {loomiePlayer && loomieGym && <Combat3D loomieUser={loomiePlayer} loomieGym={loomieGym} />}
        </View>
      </SafeAreaView>

      {/* UI */}

      {loomiePlayer && loomieGym && <CombatUI
    // state
    gym={route.params.gym} loomiePlayer={loomiePlayer} loomieGym={loomieGym} gymLoomiesLeft={gymLoomiesLeft} userLoomiesLeft={userLoomiesLeft}
    // signals
    inputAttack={userAttack} inputDodge={userDodge} inputEscape={userEscape}
    // loose modal
    modalLooseVisible={modalLooseVisible} modalLooseCallback={useCallback(() => {
      showInfoToast('Combat has ended');
      exitCombat();
    }, [])}
    // win modal
    modalWinVisible={modalWinVisible} modalWinCallback={useCallback(() => {
      showInfoToast('Combat has ended');
      exitCombat();
    }, [])}
    // use item modal
    modalItemVisible={modalItemVisible} modalItemToggle={useCallback(() => modalItemToggle(), [])} modalItemCallback={applyItem}
    // display message
    queueMessage={queueMessage} queueUpdated={queueUpdated} getMessageQueue={getMessageQueue} removeMessageFromQueue={removeMessageFromQueue}
    //Modal loomie team
    modalLoomiesTeamVisible={modalLoomiesTeamVisible} modalLoomiesTeamToggle={useCallback(() => modalLoomiesTeamToggle(), [])} getUserLoomiesTeam={userLoomiesTeam} changeLoomie={changeLoomie}
