...
export const SelectLoomieModal = ({
  ...
  const selectedLoomie = useRef<iLoomie>();
  const changePlayerLoomie = useCallback(() => {
    changeLoomie(selectedLoomie.current);
    toggleVisibilityCallback();
  }, [selectedLoomie]);
  const updateSelectedLoomie = () => {
    const loomiesWithTeamProperty = loomiesTeam?.map(loomie => {
      const isSelectedLoomie = selectedLoomie.current?._id === loomie._id ? true : false;
      return {
        ...loomie,
  ...
  const handleLoomiePress = useCallback((selectedLoomie: iLoomie) => {
    ...
    selectedLoomie.current = selectedLoomie;
  }, []);
  return <Modal isVisible={isVisible} onBackButtonPress={toggleVisibilityCallback} onBackdropPress={toggleVisibilityCallback} style={Styles.modal}>
      ...
      marginVertical: 8
        ...
        <LoomiesCombatGrid loomies={loomiesInternal || []} markIsWeakenedLoomies={true} markSelectedLoomies={true} elementsCallback={handleLoomiePress} />
      </View>
      <FloatingRedIcon onPress={changePlayerLoomie} collection='MaterialCommunityIcons' name='checkbox-marked-circle-outline' bottom={80} right={16} />
