...
export const LoomiesGrid = memo(({
  ...
  return <FlatList data={loomies} keyExtractor={item => item._id} numColumns={2} renderItem={({
    item
  }) => {
    // Sort the types lexicographically to avoid the bg suddenly changing
    item.types = item.types.sort();
    return <LoomieCard loomie={item} markIfBusy={markBusyLoomies} markIfSelected={markSelectedLoomies} cardCallback={elementsCallback} />;
  }} />;
});