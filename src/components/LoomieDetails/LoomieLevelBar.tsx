...
export const LoomieLevelBar = memo(({
  level,
  experience,
  color
}: IProps) => {
  const nextLevelRequiredExperience = getRequiredExperienceFromLevel(level + 1);
  let currentLevelPercentage = experience * 100 / getRequiredExperienceFromLevel(level + 1);
  currentLevelPercentage = Math.min(currentLevelPercentage, 100);
  return <View style={Styles.levelContainer}>
