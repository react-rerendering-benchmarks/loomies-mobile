...
export const SelectItemGrid = (props: iPropsSelectItemGrid) => {
  ...
  const toggleItemModalVisibility = (visible?: boolean) => {
    if (visible != undefined) setItemModalVisible(visible);else setItemModalVisible(!itemModalVisible);
  };
  const handleItemPress = useCallback((item: TInventoryItem) => {
  ...
  return <>
      {selectedItem && <SelectItemDetailsModal isVisible={itemModalVisible} toggleVisibility={useCallback(() => toggleItemModalVisibility(), [])} submitItem={props.submitItem} item={selectedItem} />}
      <FlatList data={props.items} keyExtractor={item => item._id} renderItem={({
      item
    }) => <ItemCard item={({
      ...item,
      type: 'item'
    } as TInventoryItem)} handleClickCallback={handleItemPress} />} numColumns={2} />
    </>;
};