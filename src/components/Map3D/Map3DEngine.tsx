...
export const Map3DEngine = memo(() => {
  ...
  useEffect(() => {
    ...
    // create player
    const playerRootMesh: Babylon.Mesh = Babylon.MeshBuilder.CreateBox('playerRootMesh', {
      size: 1
    }, sceneMap);
    ...
    // create ground plane
    const planeGround = createGradientPlane(PLANE_SIZE * GROUND_SCALE, new Babylon.Color3(...COLOR_BACKGROUND_BOT), new Babylon.Color3(...COLOR_BACKGROUND_TOP), sceneMap);
    ...
    for (let i = 0; i < GRIDMAP_SIZE; i++) {
      ...
      for (let j = 0; j < GRIDMAP_SIZE; j++) {
        ...
        if (MAP_DEBUG) {
          const boundariesPath = [new Vector3((i - gridOffset) * PLANE_SIZE, 3, (j - gridOffset) * PLANE_SIZE), new Vector3((i - gridOffset + 1) * PLANE_SIZE, 3, (j - gridOffset) * PLANE_SIZE), new Vector3((i - gridOffset + 1) * PLANE_SIZE, 3, (j - gridOffset + 1) * PLANE_SIZE), new Vector3((i - gridOffset) * PLANE_SIZE, 3, (j - gridOffset + 1) * PLANE_SIZE), new Vector3((i - gridOffset) * PLANE_SIZE, 3, (j - gridOffset) * PLANE_SIZE)];
    ...
    (async () => {
      try {
        ...
        const circleIndicatorModel = await cloneModel('MAP_CIRCLE_INDICATOR', sceneMap);
        console.log('A1');
  ...
  const UpdateTileMesh = () => {
    ...
    if (offset) {
      ...
      for (let i = 0; i < GRIDMAP_SIZE; i++) {
        for (let j = 0; j < GRIDMAP_SIZE; j++) {
          ...
          newPos.x = i + offset.x;
          newPos.y = j + offset.y;
          if (newPos.x >= 0 && newPos.x < GRIDMAP_SIZE && newPos.y >= 0 && newPos.y < GRIDMAP_SIZE) {
    ...
    getUpdatedTiles().forEach((tilePos: iGridPosition) => {
      ...
      // ROADS BORDER
      const roadsBorder = generateRoad(mapBundle.roadsBorder, COLOR_ROAD_BORDER, sceneMap);
      roadsBorder.position.y = ROAD_BORDER_HEIGHT;

      // merge them
      const finalMergedMesh = Babylon.Mesh.MergeMeshes([roads, roadsBorder], true, true, undefined, false, true);
      if (finalMergedMesh) {
        finalMergedMesh.position = new Vector3((tilePos.x - gridOffset) * PLANE_SIZE, 0, (tilePos.y - gridOffset) * PLANE_SIZE);
  ...
  const updatePlayerPos = (absolute = false) => {
    ...
    if (absolute) {
      ...
      const newPos = new Vector3((userPosition.lon - mapOrigin.lon) / (BBOX_SIZE * 2) * PLANE_SIZE, 0, (userPosition.lat - mapOrigin.lat) / (BBOX_SIZE * 2) * PLANE_SIZE);
      playerNode.current.position = new Vector3(newPos.x + (currPos.x - targPos.x), 0, newPos.z + (currPos.z - targPos.z));
      playerNodeTargetPos.current = newPos;
    }

    // set lerp target position
    else {
      playerNodeTargetPos.current = new Vector3((userPosition.lon - mapOrigin.lon) / (BBOX_SIZE * 2) * PLANE_SIZE, 0, (userPosition.lat - mapOrigin.lat) / (BBOX_SIZE * 2) * PLANE_SIZE);
    }
  };
  ...
  const frameUpdate = () => {
    ...
    // rotation
    if (Math.abs(playerNode.current.rotation.y - deviceYaw) < ANI_LERP_MERGE_DISTANCE_ROTATION) {
      playerNode.current.rotation.y = deviceYaw;
    } else {
      playerNode.current.rotation.y = Tools.ToRadians(Babylon.Scalar.LerpAngle(Tools.ToDegrees(playerNode.current.rotation.y), Tools.ToDegrees(deviceYaw), ANI_LERP_SPEED));
    }
  };
  ...
  return <>
      ...
      backgroundColor: 'black'
        ...
        flex: 1
          ...
          {!!MAP_DEBUG && <>
              <Button title={'Move x+'} onPress={() => {
            debugMovePosition({
              lon: DEBUG_MOVE_DISTANCE,
              lat: 0
            });
          }} />
              <Button title={'Move x-'} onPress={() => {
            debugMovePosition({
              ...
              <Button title={'Move y+'} onPress={() => {
            debugMovePosition({
              lon: 0,
              lat: DEBUG_MOVE_DISTANCE
            });
          }} />
              <Button title={'Move y-'} onPress={() => {
            debugMovePosition({
